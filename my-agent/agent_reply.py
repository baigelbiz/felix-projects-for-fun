...
def _parse_reminder_when(when: str, tz_name: str = "Asia/Jerusalem") -> datetime:
    ...
    # is required too: without it "at"/"ב" also match mid-word ("that 5 star",
    # "חלב 2 בקבוקים"), silently misfiring on ordinary text that never asked
...
def calendar_create_reminder(summary: str, when: str, duration_minutes: int = 15, timezone: str = "Asia/Jerusalem", account: str = "business") -> str:
    ...
    # Plain timedelta addition silently shortens/lengthens the reminder by an
    # hour if its span straddles a DST transition.
        ...
        for _ in range(10):  # max tool call rounds
            ...
            if function_calls:
                ...
                if photo_result is not None:
                    # Other tool calls requested in the same round (e.g. "remind me to call
                    # John and also make a flyer image") already ran and had real side effects
                    # (reminder created, note added, etc.) — the early return below skips the
                    # model's usual summary turn, so without this their outcome would never
                    # reach the user or the persisted history, even though it already happened.
                    if other_results:
